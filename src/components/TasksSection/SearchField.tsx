...
const ItemSearch: React.FC<{ task: Task }> = ({ task }) => {
  ...
  return (
    <li key={task.id} className="py-2">
      <Link
        to={`/task/${task.id}`}
        className="flex justify-between transition hover:text-sky dark:hover:text-sky"
      >
            ...
            {tasks.length ? (
              <>
                ...
                <button
                  ...
                  className="bg-sky w-full p-2 rounded-md text-white dark:bg-slate-700/[.3] dark:text-slate-200"
                >
